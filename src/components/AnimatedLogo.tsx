...
const ROUNDED_PATH =
  'M 0 96 Q 0 121 20 108 L 160 18 Q 190 0 160 0 L 30 0 Q 0 0 0 30 Z'
...
export function AnimatedLogo({ size = 120, href = '/' }: AnimatedLogoProps) {
  ...
  // Get current path based on rounding state and menu state
  // Use sharp path when menu is open, otherwise use rounded path
  const currentPath = menuOpen
    ? SHARP_PATH
    : isRounded
      ? ROUNDED_PATH
      : SHARP_PATH
  const currentHypotenuse = menuOpen
    ? SHARP_HYPOTENUSE
    : isRounded
      ? ROUNDED_HYPOTENUSE
      : SHARP_HYPOTENUSE

  const svgClassNames = [styles.triangleSvg, styles.triangleGrowAnimation].join(
    ' '
  )
    ...
    <div className={styles.logoWrapper}>
        ...
        >
          ...
          <path
            ...
            style={
              {
                d: `path("${currentPath}")`,
                opacity: menuOpen ? 1 : fillOpacity,
                transition: menuOpen
                  ? 'opacity 0.3s ease-out, fill 0.3s ease-out'
          ...
          <path
            ...
            d={currentPath}
            style={{ d: `path("${currentPath}")` } as React.CSSProperties}
          />
      ...
      {menuOpen && (
        <>
          ...
          >
            <svg
              className={styles.triangleMenuSvg}
              viewBox='0 0 190 121'
              preserveAspectRatio='xMinYMin meet'
            >
              <defs>
