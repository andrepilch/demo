export { Header } from './Header'
export { Footer } from './Footer'
export { DiagonalBackground } from './DiagonalBackground'
export { HeroSection } from './HeroSection'
export { HomeIntro } from './HomeIntro'
export { ImpactStatsRibbon } from './ImpactStatsRibbon'
export { BigNumbers } from './BigNumbers'
export { FixedLogo } from './FixedLogo'
export { AnimatedLogo } from './AnimatedLogo'
export {
  Text,
  H1,
  H2,
  H3,
  BodyLarge,
  Body,
  BodySmall,
  Eyebrow,
  StatNumber,
  RibbonNumber,
} from './Text'
export {
  PageWrapper,
  SectionsWrapper,
  Section,
  SectionGap,
  Container,
  SectionContent,
  SectionHeader,
  Grid,
} from './Section'
export {
  Button,
  ButtonGroup,
  PrimaryButton,
  SecondaryButton,
  OutlinedButton,
  LinkButton,
  iconSm,
  iconMd,
} from './Button'
export { LeadershipSection } from './LeadershipSection'
export {
  PhilosophyMasonry,
  PhilosophyCard,
  type PhilosophyItem,
} from './PhilosophyMasonry'
export { DesignPhilosophy } from './DesignPhilosophy'
export { AboutDesignPhilosophy } from './AboutDesignPhilosophy'
export { ProductPhilosophy } from './ProductPhilosophy'
export { CraftFocus } from './CraftFocus'
export { StrongProductBanner } from './StrongProductBanner'
export { Bio } from './Bio'
export { SkillsMatrix } from './SkillsMatrix'
export { SideProjects } from './SideProjects'
export { FeaturedCaseStudies } from './FeaturedCaseStudies'
export { CTASection } from './CTASection'
export { PassionAreas } from './PassionAreas'
export { Partnerships } from './Partnerships'
export { Accomplishments } from './Accomplishments'
export { Education } from './Education'
export { AdviceTopics } from './AdviceTopics'
export { Process } from './Process'
export { Card, type CardProps } from './Card'
export { BridgeAdvantage } from './BridgeAdvantage'
export { CareerTimeline } from './CareerTimeline'
export { AI } from './AI'
